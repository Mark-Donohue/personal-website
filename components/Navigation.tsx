...
const Navigation = () => {
  ...
  return (
    <header className="w-full mx-auto px-4 sm:px-10 bg-[#F8F8F7] shadow fixed top-0 z-50">
          ...
          <div className="flex items-center justify-between py-2 md:block">
            ...
            <div className="md:hidden">
              <button onClick={() => setSideMenu(!sideMenu)}>
                {sideMenu ? <IoClose size={36} /> : <IoMenu size={36} />}
              </button>
            </div>
          </div>
        </div>
          ...
          >
            <div className="items-center justify-center space-y-6 md:flex md:space-x-6 md:space-y-0">
              {NAV_ITEMS.map((navItem, idx) => {
                return (
                  <Link
                    className="block lg:inline-block uppercase hover:text-[#167BFF]"
                    key={idx}
                    to={navItem.section}
                    activeClass="active"
                    spy={true}
